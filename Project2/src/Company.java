...
/**
 * The Company class is a container for Employee objects. It holds all the employees in
 * the Company and handles adding, removing, setting Hours and processing payments. employees:
 * its a list of Employee objects, its a collection of all the employees currently in
 * the Company system. numEmployee: its the number of employees currently in the employees
 * array. STARTING_LIBRARY_SIZE: its the initial size of the employees array it is
...
public class Company {
   ...
   * @return index of the employee if employee is found in the employees array. otherwise -1.
   */
  private int find(Employee employee) {
    for (int i = 0; i < numEmployee; i++) {
      if (employee.equals(employees[i])) return i;
    }
    return -1;
  }

  /**
   * When called increases the size of the employees array by STARTING_COMPANY_SIZE
   */
  private void grow() {
  ...
  /**
   * Aranges the employees array to be sorted by department name
   * then prints the employees array
   *
   */
  public void printByDepartment() {
    ...
    for (int i = 0; i <= this.employees.length - 1; i++) {
      for (int j = i + 1; j <= this.employees.length - 2; j++) {
        if (i == this.employees.length - 1 || this.employees[j] == null) break; // no employees
        // I
  ...
  /**
   * Aranges the employees array to be sorted by hired date
   * then prints the employees array
   *
   */
  public void printByDate() {
    ...
    for (int i = 0; i <= this.employees.length - 1; i++) {
      for (int j = i + 1; j <= this.employees.length - 2; j++) {
        ...
        // book date at i
        int EmpHiredYearI =
          this.employees[i].getProfile().getDateHired().getYear();
        int EmpHiredMonthI =
          this.employees[i].getProfile().getDateHired().getMonth();
        int EmpHiredDayI =
          this.employees[i].getProfile().getDateHired().getDay();
        // book date at j
        int EmpHiredYearJ =
          this.employees[j].getProfile().getDateHired().getYear();
        int EmpHiredMonthJ =
          this.employees[j].getProfile().getDateHired().getMonth();
        int EmpHiredDayJ =
          this.employees[j].getProfile().getDateHired().getDay();
  ...
  public boolean alreadyExists(Employee employee) {
    if (find(employee) == -1) return false; else return true;
  }
}
